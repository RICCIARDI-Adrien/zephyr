...
static inline void can_rcar_rscanfd_configure_fifo(const struct device *dev)
{
	const struct can_rcar_rscanfd_config *config = dev->config;
	...
	can_rcar_rscanfd_write(dev, base_offset + RSCANFD_CFDCFCCN,
		...
		RSCANFD_CFDCFCCN_CFE_DISABLE << RSCANFD_CFDCFCCN_CFE_SHIFT); // TODO transmission delay dans param DT ? int

	/* Allow transmitting from the Common FIFO */
	can_rcar_rscanfd_write(dev, base_offset + RSCANFD_CFDCFCCEN,
		RSCANFD_CFDCFCCEN_CFBME_ENABLE << RSCANFD_CFDCFCCEN_CFBME_SHIFT);
...
static int can_rcar_rscanfd_start(const struct device *dev)
{
	...
	/* Reset all hardware error counters */
	...
	/* Discard the previous transmission interrupts on the channel first Common FIFO */
	base_offset = config->channel * sizeof(uint32_t) *
		CAN_RCAR_RSCANFD_COMMON_FIFO_PER_CHANNEL + RSCANFD_CFDCFSTSN;
	val = can_rcar_rscanfd_read(dev, base_offset);
	val &= ~RSCANFD_CFDCFSTSN_CFTXIF;
	can_rcar_rscanfd_write(dev, base_offset, val);

	/* Discard the previous reception interrupts on the channel second Common FIFO */
	base_offset += sizeof(uint32_t);
	val = can_rcar_rscanfd_read(dev, base_offset);
...
static int can_rcar_rscanfd_set_mode(const struct device *dev, can_mode_t mode)
{
	...
	if (mode & CAN_MODE_LOOPBACK) {
		...
		val_tx |= (RSCANFD_CFDCNCTR_CTMS_INTERNAL_LOOPBACK_MODE <<
		...
		/*
		 * Keep the TX FIFO in normal mode, the can_send() function prevents any
		 * transmission
		 */
...
static int can_rcar_rscanfd_init(const struct device *dev)
{
	...
	can_rcar_rscanfd_configure_fifo(dev);
