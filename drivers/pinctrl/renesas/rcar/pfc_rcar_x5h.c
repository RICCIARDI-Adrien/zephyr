...
/*
 * Each drive step is encoded on 3 bits.
 * So, based on a 24 mA maximum value, each step is 24/8 mA.
 */
#define PFC_RCAR_DRIVE_MIN 6
				       ...
				       uint8_t strength)
{
	...
	if (!IN_RANGE(strength, PFC_RCAR_DRIVE_MIN, PFC_RCAR_DRIVE_MAX)) {
		return -EINVAL;
	}

	/* Convert strength from mA to [0-7] */
	strength = (strength / PFC_RCAR_DRIVE_STEP) - 1;
...
static int pfc_rcar_set_bias(uintptr_t pfc_base, uint16_t pin, uint16_t flags)
{
	...
	pfc_rcar_write(pfc_base, pfc_base + bias_reg->puen, val | BIT(bit));
...
static int pinctrl_configure_pin(const pinctrl_soc_pin_t *pin)
{
	...
	if (pin->drive_strength != 0) {
		...
		settings.config[config_cnt++] = 1000 * pin->drive_strength;
	}
